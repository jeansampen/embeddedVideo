...
`,
  ...
  seekToScript: (seconds: number, allowSeekAhead: boolean) => {
  ...
  loadPlaylist: (playList: string, startIndex: number, play: boolean) => {
    ...
    const playlistArr = Array.isArray(playList)
      ? `"${playList.join(",")}"`
      : "undefined";
  ...
  loadVideoById: (videoId: string, play: boolean) => {
